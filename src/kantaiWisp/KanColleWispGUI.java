...
public class KanColleWispGUI {

    public static boolean isAutoReceiveLaunchExpo() {
        return autoReceiveLaunchExpo;
    }

    static boolean autoReceiveLaunchExpo = true;
    ...
    private KanColleWispGUI() {
        ...
        工厂Button.addActionListener(e -> {
            print("启动工厂任务");
            KantaiWispCore.factoryWisp.factoryDailies();
        });
        自动收取远程CheckBox.addActionListener(e -> {
            print("调整自动收取远征策略" + 自动收取远程CheckBox.isSelected());
            autoReceiveLaunchExpo = 自动收取远程CheckBox.isSelected();
        });
    ...
    static void initUI() {
        ...
        frame.setContentPane(new KanColleWispGUI().panel1);
